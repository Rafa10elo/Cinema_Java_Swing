...
public class MyFrame extends JFrame {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("MOVIES.txt"))) {
            ...
            while ((line=reader.readLine())!=null)
            {
                ...
                if (movieDetails.length==5) {
                    ...
                    if(id==1 && hallNumber==1){movies.add(new Movie(id,genre,name,showTime,"poster2.jpg",description));}
                    else if(id==2 && hallNumber==1){movies.add(new Movie(id,genre,name,showTime,"poster1.jpg",description));}
                    else if(id==3 && hallNumber==2){movies.add(new Movie(id,genre,name,showTime,"poster3.jpg",description));}
                    else if(id==4 && hallNumber==2){movies.add(new Movie(id,genre,name,showTime,"poster4.jpg",description));}
                    else if(id==5 && hallNumber==3){movies.add(new Movie(id,genre,name,showTime,"poster5.jpg",description));}
                    else if(id==6 && hallNumber==3){movies.add(new Movie(id,genre,name,showTime,"poster6.jpg",description));}
                }
            }
        }catch(IOException e) {
            throw new RuntimeException(e);
    ...
    private void displayMoviesForHall(int hallNumber) {
        ...
        /*moviePanelContainer.removeAll();  // Clear previous movies
        Cinema cinema = cinemaMap.get(hallNumber);
        if (cinema != null) {
            List<String> movies = cinema.getMovies();
            for (String movieName : movies) {
                JPanel moviePanel = createMoviePanel1(movieName);
                moviePanelContainer.add(moviePanel);
            }
        }
        moviePanelContainer.revalidate();
    ...
    private JPanel createMoviePanel1(String movieName) {
        ...
        infoPanel.add(new JLabel("Movie: "+movieName));

        JLabel posterLabel=new JLabel();
        ImageIcon posterIcon=new ImageIcon("poster1.jpg");
